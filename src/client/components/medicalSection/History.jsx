...
const History = ({ userId }) => {
  ...
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      try {
        const res = await axios.get(`api/patient-data/${userId}/history`);
        const responseData = res.data?.data;

        setData(Array.isArray(responseData) ? responseData[0] : responseData);
      } catch (err) {
        setError("Failed to load general history");
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) return <p>Loading general history...</p>;
  if (error) return <p className="text-red-500">{error}</p>;
  if (!data) return <p>No general history found.</p>;
