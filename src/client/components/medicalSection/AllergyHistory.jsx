...
const AllergyHistory = ({ userId }) => {
    ...
    const fetchAllergyHistory = async () => {
      try {
        const res = await axios.get(
          `api/patient-data/${userId}/allergy-history`
        );

        const responseData = res.data?.data;
        setData(Array.isArray(responseData) ? responseData[0] : responseData);
      } catch (err) {
        setError("Failed to load allergy history");
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) return <p>Loading allergy history...</p>;
  if (error) return <p className="text-red-500">{error}</p>;
  if (!data) return <p>No allergy history found.</p>;
